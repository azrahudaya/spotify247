...
- publish a lightweight GitHub Pages now-playing page
...
| GitHub Pages now-playing | Public status page | Near-live static page, generated by GitHub Actions |
...
This repo includes a lightweight static now-playing page.
...
That means GitHub Actions is asked to regenerate the page every 5 minutes. GitHub can delay scheduled runs, so treat this as near-live, not real-time.

The browser reloads `now-playing.json` every 60 seconds, but the JSON only changes after GitHub Actions generates and deploys a new copy.
